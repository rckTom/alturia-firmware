...
{
    public class Micron_MT25QL : ISPIPeripheral, IGPIOReceiver
    {
        public Micron_MT25QL(MappedMemory underlyingMemory)
        {
        ...
        public void LoadBinary(ReadFilePath filePath)
        {
            using(var reader = new FileStream(filePath, FileMode.Open, FileAccess.Read))
            {
                var buffer = new byte[underlyingMemory.Size];
                int written = 0;
                int read = 0;
                while((read = reader.Read(buffer, 0, (int)underlyingMemory.Size)) > 0) {
                    UnderlyingMemory.WriteBytes(0, buffer, read);
                    written += read;
                }
            }
        }

        public void ExportBinary(WriteFilePath filePath)
        {
            using(var writer = new FileStream(filePath, FileMode.Create, FileAccess.Write))
            {
                for(int i = 0; i < underlyingMemory.Size; i++) {
                    writer.WriteByte(underlyingMemory.ReadByte(i));
                }
            }
        }
        ...
        public byte Transmit(byte data)
        {
            ...
            {
                HandleNoDataCommand();
            } else if(currentOperation.State == DecodedOperation.OperationState.HandleImmediateCommand) {
                return 0;
            }
            return 0;
        }
        ...
        private void RecognizeOperation(byte firstByte)
        {
            ...
            switch(firstByte)
            {
                ...
                case (byte)Commands.SubsectorErase4kb:
                    ...
                    currentOperation.State = DecodedOperation.OperationState.AccumulateNoDataCommandAddressBytes;
                    break;
                case (byte)Commands.SubsectorErase4byte32kb:
                    ...
                    if(enable.Value)
                    {
                        ...
                        switch(currentOperation.EraseSize)
                        {
                            ...
                            case DecodedOperation.OperationEraseSize.Subsector4K:
                                EraseSubSector(Subsector4KSize);
                                break;
                            case DecodedOperation.OperationEraseSize.Subsector32K:
                                EraseSubSector(Subsector32KSize);
                                break;
                            default:
        ...
        private void EraseSubSector(int subsector_size)
        {
            var subsector = new byte[subsector_size];
            for (var i = 0; i < subsector_size; i++) {
                subsector[i] = EmptySegment;
            }

            var position = subsector_size * (currentOperation.ExecutionAddress / subsector_size);
            underlyingMemory.WriteBytes(position, subsector);
        }
        ...
        private readonly int Subsector4KSize = 4.KB();
        private readonly int Subsector32KSize = 32.KB();
